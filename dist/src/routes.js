...
router.patch('/cargo/:idCargo', (request, response) => __awaiter(void 0, void 0, void 0, function* () {
    const cargoRepository = typeorm_1.getCustomRepository(CargoRepository_1.CargoRepository);
    const { idCargo } = request.params;
    const cargo = yield cargoRepository.findOne({ where: { id_cargo: idCargo } });
    if (!cargo) {
        return response.status(400).json({ message: 'Cargo não existe' });
    }
    const { nivelAcesso } = cargo;
    const nivelAcessoCargo = yield CargoController_1.default.mostrarCargo(nivelAcesso);
